...
ACTIVITY_CATEGORIES = {
    "🍽️ Essen": {
        "keywords": ["frühstück", "mittagessen", "abendessen", "essen", 
                     "snack", "kaffee", "trinken", "pause", "lunch"],
        "priority": 10
    },
    "💼 Jobsuche": {
        "keywords": ["jobsuche", "bewerbung", "freelancer", "upwork", 
                     "xing", "linkedin", "karriere", "freelancermap"],
        "priority": 9
    },
    "💬 Meetings": {
        "keywords": ["diskussion", "meeting", "call", "gespräch", 
                     "besprechung", "zoom", "teams", "standup", "sync"],
        "priority": 8
    },
    "🛠️ Entwicklung": {
        "keywords": ["code", "programmieren", "entwicklung", "bug", 
                     "feature", "webapp", "app", "software", "deploy"],
        "priority": 7
    },
    "📝 Dokumentation": {
        "keywords": ["dokumentation", "docs", "readme", "anleitung", 
                     "erklärung", "beschreibung", "dokumentieren"],
        "priority": 6
    },
    "🤖 KI/Automation": {
        "keywords": ["ki", "claude", "chatgpt", "langdock", "n8n", 
                     "automation", "ai", "gpt", "gemini"],
        "priority": 5
    },
    "📚 Lernen": {
        "keywords": ["lernen", "einarbeitung", "tutorial", "kurs", 
                     "kennen gelernt", "studieren", "eingearbeitet"],
        "priority": 4
    },
    "✍️ Schreiben": {
        "keywords": ["tagebuch", "blog", "artikel", "schreiben", 
                     "notizen", "text", "geschrieben"],
        "priority": 3
    },
    "🔍 Recherche": {
        "keywords": ["recherche", "suchen", "googeln", "anschauen", 
                     "prüfen", "vergleichen"],
        "priority": 2
    },
    "❓ Sonstiges": {
...
class CategoryEngine:
    ...
    def get_category_breakdown(self, entries):
        ...
        Returns:
        {
            "🛠️ Entwicklung": {"count": 12, "percentage": 35.3, "entries": [...]},
            ...
        }
        """
        categorized = {}
        ...
        for entry in entries:
            category, score = self.categorize_activity(entry[1])
...
class AutoLearner:
    ...
    def analyze_unknown_entries(self, days=30):
        ...
        for date, day_entries in entries_by_date.items():
            for entry in day_entries:
                category, score = self.engine.categorize_activity(entry[1])
                
                # Nur "Sonstiges" oder niedrige Scores
                if category == "❓ Sonstiges" or score < 3:
                    uncategorized.append(entry[1])
                    
                    # Extrahiere 1-3 Wort-Phrasen
                    words = entry[1].lower().split()
                    for length in [1, 2, 3]:
    ...
    def _is_stopword(self, phrase):
        ...
        stopwords = ["der", "die", "das", "und", "oder", "mit", "für", 
                     "von", "zu", "im", "am", "auf", "ein", "eine"]
    ...
    def suggest_category_for_phrase(self, phrase):
        ...
        if any(indicator in phrase for indicator in time_indicators):
            suggestions.append(("🍽️ Essen", 0.8))
        ...
        if any(indicator in phrase for indicator in dev_indicators):
            suggestions.append(("🛠️ Entwicklung", 0.85))
        ...
        if any(indicator in phrase for indicator in write_indicators):
            suggestions.append(("✍️ Schreiben", 0.75))
