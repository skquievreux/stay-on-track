...
class AnalyticsEngine:
    ...
    def get_daily_summary(self, date):
        ...
        if not entries:
            return {
                ...
                "last_entry": None,
                "hours_tracked": 0,
                "effectiveness_good": 0,
                "effectiveness_bad": 0,
            }

        # Parse timestamps from dict entries
        timestamps = []
        effectiveness_good = 0
        effectiveness_bad = 0
        ...
        for entry in entries:
            try:
                # Entry is now a dict with 'timestamp' as datetime object
                timestamp = entry["timestamp"]
                if isinstance(timestamp, datetime.datetime):
                    timestamps.append(timestamp.time())
                else:
                    # Fallback if timestamp is string
                    time_obj = datetime.datetime.strptime(timestamp, "%H:%M:%S").time()
                    timestamps.append(time_obj)

                # Count effectiveness
                eff = entry.get("effectiveness")
                if eff == "good":
                    effectiveness_good += 1
                elif eff == "bad":
                    effectiveness_bad += 1
        ...
        if not timestamps:
            return {
                ...
                "effectiveness_bad": effectiveness_bad,
            }
    ...
    def get_activity_heatmap(self, days=7):
        ...
        for _date, entries in entries_by_date.items():
            for entry in entries:
                try:
                    # Entry is now a dict with 'timestamp' as datetime object
                    timestamp = entry["timestamp"]
    ...
    def get_effectiveness_stats(self, days=7):
        """Get effectiveness statistics for the period"""
        end_date = datetime.date.today()
        start_date = end_date - datetime.timedelta(days=days - 1)
        ...
        total = total_good + total_bad + total_none
        if total == 0:
            return {
                "good": 0,
                "bad": 0,
        ...
        # Calculate percentage only from rated entries
        rated = total_good + total_bad
        if rated > 0:
            good_pct = round(total_good / rated * 100, 1)
            bad_pct = round(total_bad / rated * 100, 1)
        else:
            good_pct = 0
            bad_pct = 0
    ...
    def get_overall_stats(self, days=7):
        ...
        eff_stats = self.get_effectiveness_stats(days)
        ...
        return {
            ...
            "effectiveness_good": eff_stats["good"],
            "effectiveness_bad": eff_stats["bad"],
            "effectiveness_good_pct": eff_stats["good_percentage"],
        }
